...
logger.add('debug_logs/bittrex-client-{time}.log')
RETRY_ON_ORDER_ERROR = 5  # How many times to check the order status before giving up.
...
class BittrexClient(ClientInterface):
    ...
    def _make_trade_order(self, side, symbol, amount):
        ...
        try:
            ...
            for i in range(RETRY_ON_ORDER_ERROR):
                updated_order = self.get_order(response['id'])
                if updated_order['status'] == 'CLOSED':
                    logger.debug(f'Order closed: {updated_order}')
                    break
                elif updated_order['status'] == 'OPEN':
                    logger.debug(f'Order still open: {updated_order}')
                else:
                    logger.error(f'Order failed: {updated_order}')
                    break
                time.sleep(0.5)
    ...
    def get_order(self, order_id):
        try:
            response = self._make_api_call_auth(f"/orders/{order_id}", method="GET")
            return response
        except:
