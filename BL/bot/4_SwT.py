...
TEST_MODE = True  # If set to True no orders will go to the exchange.
MAX_NUMBER_OF_CANDLES_AT_INITIALIZARION = 288  # The number of candles the bot will pull when started. Default was 288
ENABLED_QUOTE_CURRENCIES = ['USDT']  # Quote symbols to filter markets by. Leave empty to disable filtering.
ALWAYS_GENERATE_ST_BUY_SIGNAL = True # If set to True the ST indicator will always generate buy signal.
...
logger.info(f'Always generate ST buy signal: {ALWAYS_GENERATE_ST_BUY_SIGNAL}')
...
#			self.printLog("QCs: " + self.quoteAssets + ", MinDailyVolume: " + str(self.bot_parameters.minDailyVolume) + ", MinPrice: " + str(self.bot_parameters.minPrice))
	...
	def runTradingAlgorithm(self, candle, event_time):
		try:
			...
			# if a vstop signals sell, immediately exit from the trade if any
			immediateExit = False
			#if self.strategy_parameters.VSTOP_IndicatorEnabled and conditions_s[5]:
			#    immediateExit = True
	...
	def monitorEntry(self, strategyName, conditions, candle):
		try:
			...
			logger.debug(f'Trade open response: {response}')
	...
	def checkEntryForRbuy(self, signals, candles):
		try:
			...
			if not self.strategy_parameters.ST_IndicatorEnabled:
				...
				if ALWAYS_GENERATE_ST_BUY_SIGNAL:
					st = 1
				if st is not None:
					conditions[6] = st == 1
					if st == 1:
