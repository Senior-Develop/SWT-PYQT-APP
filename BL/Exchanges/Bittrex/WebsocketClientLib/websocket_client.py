...
from .constants import EventTypes, BittrexParameters, BittrexMethods, ErrorMessages, OtherConstants, BittrexStreams
...
class BittrexSocket(WebSocket):
    ...
    def _handle_connect(self):
        try:
            ...
            connection.received += self._on_received
            connection.error += self.on_error
            hub.client.on(BittrexStreams.HEARTBEAT, self._on_heartbeat)
            hub.client.on(BittrexStreams.CANDLE, self._on_candle)
    ...
    def _connection_handler(self):
        ...
        for i in range(20):
            try:
                ...
                print("Connection #", str(i + 1), "failed. Sleeping a bit...")
                time.sleep(15)
    ...
    def _handle_subscribe(self, invoke, payload):
        channel = payload[0]
        # print("handle_sub", invoke, channel)
        self.invokes.append({'invoke': invoke, 'ticker': channel})
        self.connection.corehub.server.invoke("Subscribe", channel)
        # logger.info('Successfully invoked {} [{}].'.format(invoke, ",".join(payload)))
        ...
        #     self.connection.corehub.server.invoke("Subscribe", ["heartbeat",'trade_BTC-USD'])
        #     print("hs3")
    ...
    def _handle_reconnect(self, error_message):
        ...
        try:
            ...
            for item in self.invokes:
                # print("re_handle_sub", item['invoke'], item['ticker'])
    ...
    def subscribe_to_heartbeat(self):
        invoke = BittrexStreams.HEARTBEAT
        channels = [BittrexStreams.HEARTBEAT]
        event = SubscribeEvent(invoke, channels)
        self.control_queue.put(event)

    def subscribe_to_candles(self, channels):
        invoke = BittrexStreams.CANDLE
        event = SubscribeEvent(invoke, channels)
        self.control_queue.put(event)

    # def subscribe_to_trades(self, tickers):
    #     invoke = BittrexStreams.TRADE
    #     channels = ["trade_"+ticker for ticker in tickers]
    ...
    async def _on_heartbeat(self, msg):
        msg = {'invoke_type': BittrexStreams.HEARTBEAT}
        await self.on_public(msg)

    async def _on_trade(self, args):
        msg = await process_message(args[0])
        msg['invoke_type'] = BittrexStreams.TRADE
        await self.on_public(msg)

    async def _on_candle(self, args):
        msg = await process_message(args[0])
        msg['invoke_type'] = BittrexStreams.CANDLE
        await self.on_public(msg)


    async def _on_public(self, args):
        msg = await process_message(args[0])
        msg['invoke_type'] = 'PUBLIC'
        # if 'D' in msg:
        #     if len(msg['D'][0]) > 3:
    ...
    async def _on_received(self, **kwargs):
            ...
            if invoke == BittrexMethods.GET_AUTH_CONTENT:
                ...
                for response in msg:
                    if not response['Success']:
                        print('Subscription failed: '+response['ErrorCode'])
                        print(msg)
