...
MAX_NUMBER_OF_CANDLES_AT_INITIALIZARION = 72  # The number of candles the bot will pull when started. Default was 288
...
FILTER_MARKETS_BY_VOLUME_AND_PRICE = True  # If set to True only markets above the daily volume and price parameters will be scanned.
STOP_PERCENT = 2 # Stop % - The percent from entry price / market price at which to place TSL.
EXIT_PERCENT = 5 # Exit % - Profit percent at which to exit the trade.

# USE_CPD_TRADE_EXIT = False  # If set to True trades will be closed on CPD.
...
# CPD_OPEN_OPPOSITE = False  # If set to True after a trade is closed by CPD opposite trade will be open.
...
logger.info(f'Filter markets by volume and price: {FILTER_MARKETS_BY_VOLUME_AND_PRICE}')
logger.info(f'Stop %: {STOP_PERCENT}')
logger.info(f'Exit %: {EXIT_PERCENT}')
...
STOP_PERCENT = decimal.Decimal(STOP_PERCENT)
EXIT_PERCENT = decimal.Decimal(EXIT_PERCENT)
...
#	CommissionPercentage = decimal.Decimal(0.015) #binance
        ...
        try:
            ...
            for market in markets:
                ...
                if not self.bot_parameters.runOnSelectedMarkets or market.IsSelected:
                    ...
                    # Skip markets below minimum daily volume and price parameters.
                    if FILTER_MARKETS_BY_VOLUME_AND_PRICE and (market.DailyVolume < self.bot_parameters.minDailyVolume or market.DailyPrice < self.bot_parameters.minPrice):
                        logger.info(f'Skipping market {market.Symbol}. Volume: {market.DailyVolume} Price: {market.DailyPrice}')
    ...
    def runTradingAlgorithm(self, candle, event_time):
        try:
            ...
            conditions_short = [False] * 7  # TODO: Remove if shorting is enabled.
    ...
    def monitorEntry(self, strategyName, conditions, candle, imediateEntry=False):
        try:
            ...
            # set stoploss
            if strategyName == "R_Buy":
                sl = self.subtractPercentageFromPrice(trade.EntryPrice, STOP_PERCENT)
            # elif strategyName == "S_Sell":
            #     sl = self.addPercentageToPrice(trade.EntryPrice, self.SL1Percentage)

            logger.info(f'Setting Stop % for {candle.symbol} to {sl}')
            trade.StopLoss = sl
    ...
    def monitorExit(self, trade, candle, immediateExit=False):
        try:
            if trade is None or not trade.IsOpen:
                logger.warning(f'Tried exiting closed or unexisting trade for {candle.symbol}')
            ...
            if trade.TradeType == "Buy":
                ...
                # TODO: This is a fix for the missing stop loss problem. Remove after fixed.
                if trade.StopLoss is None:
                    trade.StopLoss = trade.EntryPrice - trade.EntryPrice  * (STOP_PERCENT / 100)

                if checkForExit and trade.StopLoss != 0 and currentPrice <= trade.StopLoss:
                    shouldExit = True
                    if not trade.IsTslActivated:
                        comment = "SL Hit"
                        logger.info(f'{candle.symbol} Stop % hit on loss at {currentPrice}')
                    else:
                        comment = "TSL Hit"
                        logger.info(f'{candle.symbol} Stop % hit on profit at {currentPrice}')
                    ...
                    new_stop_loss = self.subtractPercentageFromPrice(currentPrice, STOP_PERCENT)
                    if new_stop_loss > trade.StopLoss:
                        logger.info(f'Moving Stop % for {candle.symbol} from {trade.StopLoss} to {new_stop_loss}')
                ...
                if trade.PLPercentage > EXIT_PERCENT:
                    shouldExit = True
                    logger.info(f'Exit percent for {candle.symbol} hit at {currentPrice}.')
                ...
                logger.debug(f'Updating {candle.symbol} trade in db.')
                dnTrade.updateTrade(trade)
            ...
            logger.error(f'While exiting trade for {candle.symbol}: {err}')
            self.printLog("Exception: monitorExit", err)
